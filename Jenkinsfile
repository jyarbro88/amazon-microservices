    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                ...
                sh 'cf push accounts --random-route -p applications/accounts/build/libs/accounts-0.0.1-SNAPSHOT.jar'
                sh 'cf push orders --random-route -p applications/orders/build/libs/orders-0.0.1-SNAPSHOT.jar'
                sh 'cf push products --random-route -p applications/products/build/libs/products-0.0.1-SNAPSHOT.jar'
                sh 'cf push shipments --random-route -p applications/shipments/build/libs/shipments-0.0.1-SNAPSHOT.jar'
            }
        }
    }
}